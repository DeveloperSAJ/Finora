...
const getDailyReport = async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: "Server Error in getDailyReport",
      error: error.message,
    });
  }
};
...
const getMonthlyReport = async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: "Server Error in getMonthlyReport",
      error: error.message,
    });
  }
};
...
const getDailyTrend = async (req, res) => {
  try {
    ...
    const savingsResult = await pool.query(
      ...
      SELECT 
        EXTRACT(DAY FROM transaction_date)::int AS day,
        COALESCE(SUM(CASE WHEN type = 'deposit' THEN amount ELSE 0 END), 0) AS savings_deposit,
        COALESCE(SUM(CASE WHEN type = 'withdrawal' THEN amount ELSE 0 END), 0) AS savings_withdrawal
      FROM savings
      WHERE user_id = $1
        AND transaction_date >= $2
        AND transaction_date <= $3
    ...
    savingsResult.rows.forEach((r) => {
      // Net savings for the day (deposit - withdrawal)
      map[r.day].savings =
        parseFloat(r.savings_deposit) - parseFloat(r.savings_withdrawal);
    });
