...
const Savings = () => {
  ...
  const exportSavingsPDF = async () => {
    ...
    try {
      ...
      pdf.setFontSize(22);
      pdf.text("Finora Savings Report", pageWidth / 2, yPosition, {
        align: "center",
      });
      yPosition += 15;
      ...
      filteredSavings.slice(0, 30).forEach((s) => {
        ...
        pdf.text(
          `${s.type === "deposit" ? "+" : "-"}Rs ${parseFloat(s.amount).toLocaleString()}`,
          170,
          yPosition,
        );
        yPosition += 8;
    ...
    <div className="space-y-5 md:space-y-8">
      ...
      <Card>
        <div className="text-center py-8 md:py-12">
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-2 text-sm md:text-lg">
            Total Savings
          </p>
        </div>
      </Card>
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  <div
                    ...
                    ${s.type === "deposit" ? "bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600" : "bg-red-100 dark:bg-red-900/30 text-red-600"}`}
                  >
                    {s.type === "deposit" ? "↑" : "↓"}
                  </div>
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <p
                    className={`font-bold text-sm md:text-base ${s.type === "deposit" ? "text-emerald-600" : "text-red-600"}`}
                  >
                    {s.type === "deposit" ? "+" : "-"}Rs{" "}
          ...
          <div className="bg-white dark:bg-gray-900 rounded-3xl p-6 md:p-8 max-w-md w-full">
            ...
            <div className="bg-gray-50 dark:bg-gray-800 rounded-2xl p-4 mb-6">
              <p className="font-medium text-gray-800 dark:text-white text-sm">
                {savingToDelete.description}
              </p>
              <p
                className={`text-base font-bold mt-1 ${savingToDelete.type === "deposit" ? "text-emerald-600" : "text-red-600"}`}
              >
                {savingToDelete.type === "deposit" ? "+" : "-"}Rs{" "}
                {parseFloat(savingToDelete.amount).toLocaleString("en-IN")}
              </p>
            </div>
